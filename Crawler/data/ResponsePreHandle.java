14
https://raw.githubusercontent.com/FanChael/MVVM/master/librarys/lib_network/src/main/java/com/hl/lib_network/net/response/ResponsePreHandle.java
...
public class ResponsePreHandle<T, W> implements Function<HttpResponse<W>, T> {
    ...
    public T apply(HttpResponse<W> wHttpResponse) {
        ...
        } else if (2 == wHttpResponse.getErrorCode()) {
            ///< Token过期
            throw new ExceptionHandle.ServerException(ExceptionHandle.ERROR.TOKEN, wHttpResponse.getErrorMsg());
        } else if (11 == wHttpResponse.getErrorCode()) {
            ///< 需要绑定账号(第三方登录)
            throw new ExceptionHandle.ServerException(ExceptionHandle.ERROR.THIRD_BIND, wHttpResponse.getErrorMsg());
        }
        else {
            ///< 其他情况 - 返回真实的message信息
